...
from demo.models import get_config_dir
...
def save_lessons(store: LessonStore, path: Path | None = None) -> None:
    """Atomically write *store* to *path* (default ``get_config_dir() / "demo-lessons.yaml"``).
...
def extract_lessons(result: DemoEvalResult) -> list[str]:  # noqa: F821 — forward ref
    ...
    runs return ``[]``.
    """
    from demo.models import DemoEvalResult  # local import — avoid circular
