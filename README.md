...
Each axiom produces derived implications — currently 90 across all tiers. An implication is a specific, enforceable constraint derived from the general axiom. The axiom `executive_function` says the system should reduce cognitive load; the implication `ex-err-001` says error messages must include a concrete next action — a command to run, a file to check, a service to restart. Implications are enforced at four tiers:
